...
-- The flagship feature of this script is the network_song_player. It is essentially a wrapper around SongPlayer,
-- but it ensures all control functions are synced over pings to viewers and the host.
--
...
local function receive_data_packet(transferred_song_id, packet_data_string)
    if not collected_incoming_songs[transferred_song_id] then
        if not missed_incoming_songs[transferred_song_id] then
            print_debug("Received a data packet for song with transfer ID `"..tostring(transferred_song_id).."` before receiving a header packet for the song. Future lost packets for this song will be ignored.")
            missed_incoming_songs[transferred_song_id] = true
        end
        return
    end

    packet_decoder_api.add_instructions_to_song_from_packet(
        collected_incoming_songs[transferred_song_id].song,
        packet_data_string
    )
end
...
---@param transferred_song_id integer
---@param packet_data_string PacketDataString
local function receive_config_packet(transferred_song_id, packet_data_string)
    if not collected_incoming_songs[transferred_song_id] then
        if not missed_incoming_songs[transferred_song_id] then
            print_debug("Received a config packet for song with transfer ID `"..tostring(transferred_song_id).."` before receiving a header packet for the song. Future lost packets for this song will be ignored.")
            missed_incoming_songs[transferred_song_id] = true
        end
        return
    end
    ...
    collected_incoming_songs[transferred_song_id].player.set_new_config(new_config)
end

-- A set of functions to call if we receive a header packet.
--
-- Allows us to do some host-only initialization. Technically a viewer script may also try to call these, but it's intended to just finish some inits for host-side network player
--
...
---@param transferred_song_id integer
---@param packet_data_string PacketDataString
local function receive_header_packet(transferred_song_id, packet_data_string)
    ...
    local callback = on_header_received_functions[transferred_song_id]
    if callback then
        callback(new_player_controller)
        on_header_received_functions[transferred_song_id] = nil
    end
    return new_song
end
...
---@param transferred_song_id integer
---@param packet_data_string PacketDataString
local function receive_control_packet(transferred_song_id, packet_data_string)
    if not collected_incoming_songs[transferred_song_id] then
        if not missed_incoming_songs[transferred_song_id] then
            print_debug("Received a control packet for song with transfer ID `"..tostring(transferred_song_id).."` before receiving a header packet for the song. Future lost packets for this song will be ignored.")
            missed_incoming_songs[transferred_song_id] = true
        end
        return
    end

    packet_decoder_api.control_player_from_packet(
        collected_incoming_songs[transferred_song_id].player,
        packet_data_string
    )
end

-- function lookup table for packet receiver
---@type table<PacketTypeIDs, fun(transferred_song_id: integer, packet_data_string:PacketDataString)>
...
--- By running in a tick event, we ensure that, on the off chance we receive two pings on the same tick, that we process them on different ticks.
---@see local_receive_packet
local function local_receive_packet_loop()
    ---@type BundledPacket
    local incoming_bundled_packet = table.remove(incoming_packed_packets, 1)   -- table.remove is usually inefficient when popping from the front. But we shouldn't have more than like 2 packets in here at a time, so should be fine.
    packet_receiving_functions[incoming_bundled_packet.packet_type](incoming_bundled_packet.transferred_song_id, incoming_bundled_packet.packet_data_string)
...
---
--- Pings appear to have their own instruction limits separate from TICK or RENDER or whatever, but this limit isn't displayed anywhere.
...
--- cost of pings can occasionally double without warning.
---
--- Dispatch to our own TICK loop, so that we can control when we process these pings, and prevent doubling up.
---
...
---@param packet_data_string PacketDataString
local function local_receive_packet(transferred_song_id, packet_type, packet_data_string)
    ---@type BundledPacket
    local bundled_packet = {
        transferred_song_id = transferred_song_id,
...
--- On the off chance that pings need to be unique (I'm not sure at the moment): `TL_FMP` → Tanner Limes Figura Music Player
---
--- if skip_queue is true, then skip the packet processing queue and processes the packet immediately. Will use the unlisted PING event.
--- Because packets are sometimes merged, actual instruction cost is unpredictable. Use sparingly, prefer tick events where possible.
---@param transfer_id integer
---@param packet_type PacketTypeIDs
...
--- Essentially a wrapper for pings.TL_FMP_receive_packet
---
--- Bypasses the outbound packet queue. Good for control codes, bad for bulk data.
---
---@param transfer_id integer
---@param packet_type PacketTypeIDs
---@param outgoing_packed_packet PacketDataString
---@param skip_queue boolean?   -- Skip the viewer-side dequeue process and ensures it's processed immediately by the viewer. Typically not ideal.
local function ping_packet_immediately(transfer_id, packet_type, outgoing_packed_packet, skip_queue)
    pings.TL_FMP_receive_packet(transfer_id, packet_type, outgoing_packed_packet, skip_queue)
end

---@alias BundledPacket {transferred_song_id: integer, packet_type: PacketTypeIDs, packet_data_string: PacketDataString}    -- a light weight way to keep a packet tied to it's packet ID and transfer ID.
...
--- Searches upcoming packets in outgoing_packet_queue and skips/removes any that match transferred_song_id_to_cancel
---
--- Only really useful if there are multiple songs in the queue, which should never happen if the HOST is only using ui.lua.
--- But if the host is doing something clever with multiple players, or multiple UIs, this fn is necessary.
---
--- Item removal logic based on https://stackoverflow.com/a/53038524
---@see stop_and_cleanup_packet_ping_loop
---@param transferred_song_id_to_cancel integer
local function remove_packets_from_outgoing_queue_by_transfer_id(transferred_song_id_to_cancel)
    ...
    for search_index =
        ...
        #outgoing_bundled_packets_queue
    do
        local should_delete_packet = outgoing_bundled_packets_queue[search_index].transferred_song_id == transferred_song_id_to_cancel
        if not should_delete_packet then
            ...
            if outgoing_packet_queue_index == search_index then
                ...
                ping_loop_start_time = ping_loop_start_time + packet_encoder_api.get_target_ms_between_packets() -- advance ping_loop_start_time to account for moved index
            else
                outgoing_bundled_packets_queue[search_index] = nil
    ...
    -- Any nils we created should have propagated to the end of the queue by now.
    -- But since lua considers the length to the index of the last non-nil value,
    -- it effectively means the list has shrunk, so #outgoing_packet_queue should
    -- accurately represent the new size.
...
--- (outgoing_packet_queue_index) rather than to actually remove packets from the list.
--- But this does mean that outgoing_packet_queue will constantly grow since no items
--- are removed.
---
--- Typically this isn't really a problem, because the queue gets reset  whenever we
...
local function remove_already_sent_packets_from_outgoing_packet_queue()
    ...
    ping_loop_start_time = ping_loop_start_time + (packet_encoder_api.get_target_ms_between_packets() * (outgoing_packet_queue_index -1))
    outgoing_packet_queue_index = 1
...
local function ping_loop()
    while ping_loop_start_time + (packet_encoder_api.get_target_ms_between_packets() * (outgoing_packet_queue_index -1)) < client:getSystemTime() do
        ...
        -- early will avoid the "slip" caused from missing the perfect time to emit a packet.
        ...
        pings.TL_FMP_receive_packet(
            outgoing_bundled_packets_queue[outgoing_packet_queue_index].transferred_song_id,
...
---
--- Probably unnecessary. most of the time that we want to ping something,
...
local function new_network_song_player(outbound_song, outbound_player_config)
    ...
    -- Initialize remote player on host so the controller can be initialize.
    -- We don't need to initialize this on the viewers just yet because we'll always reset it on :play() anyways.
    ...
    local duration_to_wait_before_assuming_play_or_stop_failed = math.min(
    ...
    custom_song_controller = {
        play = function()
            ...
            if #outgoing_bundled_packets_queue > 0 then
                print_debug("The outgoing packet queue is already busy. Playback might be delayed.")
            end

            -- We have to re-initialize the song in case someone new has loaded us for the first time.
            ping_packet_immediately(
                transferred_song_id,
                packet_enums_api.packet_type_ids.header,
                header_packet_data,
                true
            ...
            on_header_received_functions[transferred_song_id] = function (new_song_controller)
        ...
        stop = function()
            ...
            remove_packets_from_outgoing_queue_by_transfer_id(transferred_song_id)
        ...
        is_playing = function()
            local local_player_is_playing = collected_incoming_songs[transferred_song_id].player.is_playing()
            if local_player_is_playing then -- Local player is playing. Double check that we didn't just send a stop command.
                local we_stopped_playing_recently = time_stop_last_called and (time_stop_last_called + duration_to_wait_before_assuming_play_or_stop_failed > client:getSystemTime())
                ...
                local we_started_playing_recently = time_play_last_called and (time_play_last_called + duration_to_wait_before_assuming_play_or_stop_failed > client:getSystemTime())
    ...
    for k, v in pairs(collected_incoming_songs[transferred_song_id].player) do -- loop through the functions of a known player. At this stage, this player will be the wrong player. But it holds the keys we need.
        if not custom_song_controller[k] then -- there's a function we haven't implemented yet
            ...
            if type(v) == "function" then
                custom_song_controller[k] = function(...)
                    -- forwards control to the current local player
                    return collected_incoming_songs[transferred_song_id].player[k](...)
                end
            end
        end
    end
...
local api = {
    ...
    ---@param transferred_song_id integer
    stop_transferred_song = function(transferred_song_id)
        ping_packet_immediately(
            transferred_song_id,
            packet_enums_api.packet_type_ids.control,
            packet_encoder_api.make_control_packet(control_packet_codes.stop)
        )
        remove_packets_from_outgoing_queue_by_transfer_id(transferred_song_id) -- Does not cancel the above packet, since ping_packet_immediately bypasses the outgoing packet queue
    end,

    --- Effectively deletes a transferred song on all viewers.
    ---
    --- To reuse this song, a new header packet will need to be sent.
    ---@param transferred_song_id integer
    remove_transferred_song = function(transferred_song_id)
        ping_packet_immediately(
            transferred_song_id,
            packet_enums_api.packet_type_ids.control,
    ...
    ---@param transferred_song_id integer
    ---@return SongPlayerController?
    get_player_for_transferred_song = function(transferred_song_id) return collected_incoming_songs[transferred_song_id] and collected_incoming_songs[transferred_song_id].player or nil end,

    ---@return number get_target_ms_between_packets
    get_target_ms_between_packets = function() return packet_encoder_api.get_target_ms_between_packets() end,
}
