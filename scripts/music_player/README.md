...
The intended entrypoint is the `core.build_default_ui_action()` function in `core.lua`. Check out the `action_wheel.lua` script in the directory above to see how it's intended to be used. 
...
- The selected song is marked with an `⏳` to say it is being processed. It will eventualy turn into a `✓` to signal it's ready. 
...
- Some part of the script called a Midi song's data processor function. 
  - `midi_processor.lua` Spins up some initilization, then kicks off a WorldRender event. 
    - The data processor is very heavy. On somewhat large songs, it cant freeze the game for a good chunk of time. By leverageing the event loop, we can make this process asyncronous. 
    - We use a WorldRender event because the World events are allways run, and the render events don't try to "catch up" if they take too long. (Tick events eventualy stack up and the game will freeze as it tries to process ticks back to back.) This lets us let go of controll every now and then, let the main game thread catch up, and then pick up on the next available frame. This is important for 
  - For the benifit of the rest of the script, the data processor immediatly returns a future `see futures.lua` that the world event will update.
...
- Meanwhile, the data processor loop (running on the WorldRender event loop)
  - Steps through some phases to read through a midi file. (see `MidiProcessorStageKey` and `midi_processor_loop_stage_functions`)
    ...
    - `read`: reads in data a few bytes at a time. Splits the file into its sepperate chunks, and will process header chunks if it finds one.
    - `process`: looks through all track chunks and finds the next event (chronologicaly). It then sends the event to `midi_message_functions` for further processing
...
1. Starts it's data processor loop on Init, start data_processor actualy just returns a pre-created future for that song. 
2. Is designed to run on all clients. Each step is actualy very minimal and runs on the TICK event. This means that all clients will (eventualy) load every local song. 

`LocalSongScripts` are essentialy a lua script that just stores all the `PacketDataString`s nessesary to build a song useing `packet_decoder.lua`. We use this system because we know all the the decoder functions are already light enough to run on the viewer, and `PacketDataString`s are pretty well compressed too. Not as good as MIDI, but competitive with ABC. 
...
The Tick event:
- `local_songs.lua` steps through a set of functions. Each function is responcible for processing all songs, and eventualy advancing to the next function. see `local_song_tick_loop_functions`. If theres an error at any step, the problem song is removed from the queue and it's future will be set to done_with_error.
...
Anything calling a local song's process_data function will be given the Future created at Init. This future may already be done by the time it's requested, but the callback functions should still work. They'll just be called immediatly instead of eventualy.
...
- We need to stay well under the [Ping system](https://figura-wiki.pages.dev/tutorials/Pings)'s size, frequency limits, and total data rate. (we might be shareing resources with other scripts in this avatar.)
...
- makeing sure all song packets go through one bottleneck: `pings.TL_FMP_receive_packet()`.
...
`pings.TL_FMP_receive_packet()` primaraly works with the `packet_receiving_functions`. These look at the packet_type and decide what to do from there.
...
`networking.lua` also provides a `new_network_song_player()` function that wraps everything up into the same interface as a `SongPlayerController`, but it ensures all audible actions are synced to viewers. It is the reccomended way to play songs and sync them over the network. (Do note that not all actions are synced. Notably the callback functions only run on the Host.)

### Playback loop

Once a song is processed, it can be given to `song_player.lua` → `new_player()` to create a new SongPlayer. SongPlayers are always initilized with a song, and only have one song. If you want to implement a playlist of songs, you'll need to coordinate multiple players together. 

(For syncing songs between clients, use `networking.lua` → `new_network_song_player()` instead. They share roughly the same API.)

`new_player()` actualy returns a SongPlayerController. This helps protect SongPlayer from accidental modifications and keeps the LSP's output a lot cleaner. 
...
Song players actualy use multiple event loops to manage themselves. 
...
By default the main loop uses the Render event to get better temporal resolution. (Unless the viewer is running at 20FPS, the Render event is much faster than the Tick event.) But the Render event doesn't allways run. This commonly happens when the avatar is off-screen on low and default permissions, but it can also happen with some modded full-screen GUIs like Xaero's Worldmap. So all song_players are set up with a fallback event. By default the fallback event is the Tick event, which is usualy pretty reliable, and has a high instruction limit. The WorldTick watcher loop is in charge of noticeing when the main event fails, and to switch to the fallback.
...
The emergency stop runs through a few diffrent steps. 
...
- `emergency_stop_active_instruments`: One instrument per tick, disable one note in one instrument, untill all instruments say they are finished.
...
This project uses a special data format for storeing songs. Songs have some metadata like name, buffer time, song durration, etc. But also a list of Instructions. An instruction has all the information for one note. Its start time (reletive to the start of the song), its durration, the note number, track number, and modifiers for that note. Unlike MIDI where a note can be started and then we just have to wait for the "note stop" event, instructions allways keep these values together. This makes them safe to send over pings. Even if we drop packets, we won't fail to clean up sounds we started. 

There is one exception to the always-together rule: to improve buffer times, modifiers may be sepperated from an instruction and sent later as a sepperate "modifier instruction." This kind of instruction only exists in the networking/packets system. Not all instruments support all modifier types (frankly there's only like 2 supported modifiers anyways), so they are treated as second class citizens. 
