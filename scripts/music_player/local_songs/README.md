...
Local songs are a special custom song format that are essentially just a stream of packet strings exported as Lua files.

This allows us to read in local files extremely efficiently by simply calling require() on them, and letting the lua interpreter just handle everything.
